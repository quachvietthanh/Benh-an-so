 ...
 * Integrates doctor working hours (weekly recurring or date-specific overrides),
...
public class GetDoctorWeeklyScheduleTableService implements GetDoctorWeeklyScheduleTableUseCase {
    ...
    public DoctorWeeklyTableResult getWeeklyScheduleTable(GetDoctorWeeklyScheduleTableQuery query) {
        ...
        List<Appointment> appointments = appointmentRepository
                .findAppointmentsForDoctorsBetween(doctorIds, weekStartInstant, weekEndInstant, DISPLAY_STATUSES);
        ...
        for (int dayOffset = 0; dayOffset < 7; dayOffset++) {
            ...
            for (User doctor : doctors) {
                ...
                List<DoctorTimeOff> doctorTimeOffs = timeOffsByDoctor.getOrDefault(docId, List.of()).stream()
                        .filter(to -> to.overlaps(
                                currentDate.atStartOfDay(CLINIC_ZONE).toInstant(),
                                currentDate.plusDays(1).atStartOfDay(CLINIC_ZONE).toInstant()
                        ))
                        .toList();
                ...
                List<Appointment> doctorAppointments = appointmentsByDoctor.getOrDefault(docId, List.of()).stream()
                        .filter(a -> overlaps(
                                a,
                                currentDate.atStartOfDay(CLINIC_ZONE).toInstant(),
                                currentDate.plusDays(1).atStartOfDay(CLINIC_ZONE).toInstant()
