...
public class VitalSignAuthorizationService {
    ...
    public UUID requireReadAccess() {
        if (!currentUserPort.hasRole("ADMIN") && !currentUserPort.hasRole("DOCTOR")) {
            throw new MedicalRecordAccessDeniedException("Bạn không có quyền xem chỉ số sinh tồn.");
        }
    ...
    public UUID requireWriteAccess() {
        if (!currentUserPort.hasRole("ADMIN") && !currentUserPort.hasRole("DOCTOR")) {
            throw new MedicalRecordAccessDeniedException("Bạn không có quyền ghi chỉ số sinh tồn.");
        }
        return currentUserPort.getCurrentUserId();
    }
