...
-- Seed comprehensive demo data for 78 User Stories:
...
SELECT UUID_TO_BIN('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaa14'),
       ...
       NULL,
       CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = 'doctor_new');
...
SELECT UUID_TO_BIN('ea000000-0000-0000-0000-000000000001'),
       ...
       NULL,
       CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM patient_allergies WHERE id = UUID_TO_BIN('ea000000-0000-0000-0000-000000000001'));
...
SELECT UUID_TO_BIN('ea000000-0000-0000-0000-000000000002'),
       ...
       NULL,
       CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM patient_allergies WHERE id = UUID_TO_BIN('ea000000-0000-0000-0000-000000000002'));
...
SELECT UUID_TO_BIN('fa000000-0000-0000-0000-000000000001'),
       ...
       UUID_TO_BIN('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa2'),
       CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM follow_up_reminders WHERE id = UUID_TO_BIN('fa000000-0000-0000-0000-000000000001'));
...
SELECT UUID_TO_BIN('fb000000-0000-0000-0000-000000000001'),
       ...
       CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM post_care_logs WHERE id = UUID_TO_BIN('fb000000-0000-0000-0000-000000000001'));
...
SELECT UUID_TO_BIN('fc000000-0000-0000-0000-000000000001'),
       ...
       'LOGIN_BRUTE_FORCE',
       'HIGH',
       'Phat hien 5 lan dang nhap sai lien tiep tu IP 192.168.1.100',
       5,
       CURRENT_TIMESTAMP,
       CURRENT_TIMESTAMP,
       'NEW',
       CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM security_alerts WHERE id = UUID_TO_BIN('fc000000-0000-0000-0000-000000000001'));
