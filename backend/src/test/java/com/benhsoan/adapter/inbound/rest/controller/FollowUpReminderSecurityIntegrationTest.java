...
@Import({FollowUpReminderRestMapper.class, SecurityConfig.class, JwtAuthenticationFilter.class})
class FollowUpReminderSecurityIntegrationTest {
    ...
    void allowsAdminAndReceptionist() throws Exception {
        ...
        for (String role : new String[]{"ADMIN", "RECEPTIONIST"}) {
            ...
            mockMvc.perform(get("/follow-up-reminders").with(user(role.toLowerCase()).roles(role)))
                    .andExpect(status().isOk());
