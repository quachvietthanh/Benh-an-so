...
class ActivateUserServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new ActivateUserService(userRepository, roleRepository, userResultMapper,
                adminOperationAuditService, currentUserPort, clockPort, loginAttemptPort);
    }

    @Test
    void activatingInactiveAccountRecordsRealStateTransition() {
        ...
        when(userResultMapper.toResult(any(User.class), eq(role))).thenReturn(
                new UserResult(userId, "doctor1", "Doctor One", "d@x.com", null, "DOCTOR", true));
        ...
        assertEquals(Boolean.FALSE, before.getValue().get("active"));
        assertEquals(Boolean.TRUE, after.getValue().get("active"));
    }

    @Test
    void activatingAlreadyActiveAccountDoesNotRecordMisleadingTransition() {
        UUID userId = UUID.randomUUID();
        UUID roleId = UUID.randomUUID();
