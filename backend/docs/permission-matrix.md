...
| `PUT /medical-records/{id}/diagnoses` → `replaceDiagnoses` | `MEDICAL_RECORD_UPDATE` | NCL-13-CN-002: chỉ `DOCTOR` được ghi; `ADMIN` bị loại trừ. Bệnh án phải sửa được, lượt khám active, chẩn đoán chính bắt buộc mã bệnh active; danh sách chẩn đoán phụ tùy chọn và mỗi mục có thể chọn mã hoặc nhập tự do. | Đã triển khai và có acceptance-test coverage |
...
| `POST /visits/{visitId}/handover` → `VisitController.handover` | `MEDICAL_RECORD_HANDOVER` | NCL-04-CN-014: Bác sĩ phụ trách lượt khám hoặc ADMIN; visit chưa hoàn thành/hủy; bệnh án chưa ký/khóa (TC-02); bác sĩ nhận phải là DOCTOR active khác bác sĩ hiện tại | V67; seeded to `DOCTOR`, `ADMIN` |
| `GET /visits/{visitId}/handovers` → `VisitController.getHandovers` | `MEDICAL_RECORD_READ` | Bác sĩ phụ trách, bác sĩ ban đầu, các bác sĩ tham gia bàn giao trong lượt khám hoặc ADMIN | V67 |
| `GET /visits/handover/doctors` → `VisitController.getHandoverDoctors` | `MEDICAL_RECORD_READ` | Lấy danh sách các bác sĩ đang hoạt động để phục vụ bàn giao | V67 |
