...
| `POST /clinical-orders/{orderId}/cancel`; `POST /clinical-orders/items/{itemId}/cancel` | `NEW: CLINICAL_ORDER_CANCEL` | Bác sĩ phụ trách / người chỉ định / ADMIN; visit active, medical record open/draft, không có kết quả (QTN-13) | V60; seeded to `DOCTOR`, `ADMIN` |
