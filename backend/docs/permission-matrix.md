...
| `GET /inventory/reports/in-out-stock` → `InventoryReportController.getReport` | `PHARMACY_READ` OR `REPORT_VIEW` | NCL-06-CN-013: Khoảng ngày hợp lệ (tối đa 366 ngày); Contextual Authorizer: chỉ `PHARMACIST`, `MANAGER`, `ADMIN`; từ chối `DOCTOR`, `RECEPTIONIST` | Implemented — NCL-06-CN-013 |
| `GET /inventory/reports/in-out-stock/export` → `InventoryReportController.export` | `PHARMACY_READ` OR `REPORT_EXPORT` | NCL-06-CN-013: Khoảng ngày hợp lệ; Contextual Authorizer: chỉ `PHARMACIST`, `MANAGER`, `ADMIN`; xuất file CSV UTF-8 BOM kèm audit | Implemented — NCL-06-CN-013 |
