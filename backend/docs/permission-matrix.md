...
|---|---|---|---|
| `GET /medical-records/{id}/template-options`, `/visits/{visitId}/template-options` → template-selection methods | `MEDICAL_RECORD_READ` | Chỉ `DOCTOR` là bác sĩ phụ trách visit; resolve active template theo `Visit.specialtyId`, hoặc GENERAL default duy nhất (`fallback=true`) | Implemented — NCL-13-CN-004 |
| `PUT /medical-records/{id}/template` → `applyTemplate` | `MEDICAL_RECORD_UPDATE` | Chỉ `DOCTOR` là bác sĩ phụ trách, visit active, record editable; không đổi khi đã có nội dung; template active/cùng specialty hoặc GENERAL fallback xác định | Implemented — NCL-13-CN-004 |
...
| `POST /patients/merge` → `PatientController.mergePatients` | `PATIENT_MERGE` | NCL-02-CN-006: RECEPTIONIST, MANAGER, ADMIN. DOCTOR bị từ chối 403. Kiểm tra trùng/khóa định danh QTN-33, cập nhật hồ sơ nguồn thành MERGED (read-only), chuyển toàn bộ lượt khám, đơn thuốc, chỉ định, công nợ sang hồ sơ đích | Đã triển khai và có acceptance-test coverage |
| `GET /patients/duplicates` → `PatientController.findDuplicates` | `PATIENT_READ` | NCL-02-CN-006: Tìm kiếm danh sách các nhóm hồ sơ nghi trùng theo họ tên, ngày sinh, số điện thoại | Đã triển khai và có acceptance-test coverage |
...
| `GET /visits/{visitId}/encounter` → `VisitController.getEncounter` | `MEDICAL_RECORD_READ` | ADMIN đọc; DOCTOR chỉ khi là bác sĩ phụ trách visit | Permission + service context |
| `POST /medical-records` → `create`; `PUT /medical-records/{id}` → `update`; `PUT /medical-records/{id}/instructions-and-treatment-plan` → `updateInstructionsAndTreatmentPlan`; `POST /medical-records/{id}/lock` → `lock`; `POST /medical-records/{id}/amendments` → `amend` | `MEDICAL_RECORD_CREATE` / `MEDICAL_RECORD_UPDATE` / `MEDICAL_RECORD_UPDATE_STATUS` (lock) | Giữ `MedicalRecordAuthorizationService`, trạng thái lock/amend và actor | Chuyển role tổng quát, giữ service context |
| `PUT /medical-records/{id}/diagnoses` → `replaceDiagnoses` | `MEDICAL_RECORD_UPDATE` | NCL-13-CN-002 & NCL-13-CN-006: chỉ `DOCTOR` được ghi; `ADMIN` bị loại trừ. Bệnh án phải sửa được, lượt khám active, chẩn đoán chính bắt buộc mã bệnh active theo QTN-22; danh sách bệnh mắc kèm (secondary diagnoses) tùy chọn, hỗ trợ chọn nhiều mã bệnh hoặc nhập tự do, chặn trùng lặp mã bệnh hoặc tên tự do (TC-01, TC-02, TC-03). | Đã triển khai và có acceptance-test coverage |
| `GET /medical-records/{id}`, `/{id}/diagnoses`, `/visits/{visitId}`, `/patient/{patientId}` → read methods | `MEDICAL_RECORD_READ` | NCL-13-CN-006: `GET /{id}/diagnoses` hỗ trợ lọc theo loại chẩn đoán (`type=PRIMARY` hoặc `type=SECONDARY`), mặc định sắp xếp chẩn đoán chính lên trước; trả về đầy đủ `diagnosisCatalogId`. Giữ phạm vi hồ sơ/bệnh nhân. | Chuyển từ role |
...
| `POST /inventory/batches/{id}/adjust`, `POST /inventory/batches/{id}/discard` → `InventoryController.adjustBatchStock`, `discardExpiredBatch` | `PHARMACY_UPDATE` | NCL-06-CN-010: Controller yêu cầu `PHARMACY_UPDATE`; Service yêu cầu `PHARMACY_UPDATE` hoặc vai trò `PHARMACIST`/`ADMIN` qua `requireInventoryUpdate()` (từ chối `PHARMACY_READ`); yêu cầu lý do (QTN-32); discard chỉ khi hết hạn (QTN-14); ghi nhận chênh lệch StockMovement và cập nhật tồn catalog | Đã triển khai và có acceptance-test coverage |
...
| `GET /queues` → `QueueController.getQueues`; `GET /queue-items/{itemId}` → `getById`; `GET /queue-items/{itemId}/history` → `getHistory` | `QUEUE_VIEW` | Phạm vi ngày/bác sĩ/queue; lịch sử hàng đợi (hỗ trợ ADMIN, RECEPTIONIST, MANAGER theo AC-04, và DOCTOR phụ trách) | Chuyển từ role |
...
| `PATCH /queue-items/{itemId}/status`; `POST /queue-items/{itemId}/complete`; `POST /queue-items/{itemId}/skip`; `POST /queue-items/{itemId}/re-queue`; `POST /queue-items/{itemId}/prioritize` | `QUEUE_UPDATE_STATUS` | Chỉ transition hợp lệ; bác sĩ phụ trách/role workflow theo `QueueOperationAuthorization` (re-queue và prioritize cho ADMIN, RECEPTIONIST; prioritize yêu cầu lý do theo QTN-40 và trạng thái WAITING) | Permission + service context |
...
| `GET /system/specialties`; `GET /system/medical-record-templates`, `/{id}`; `POST /system/medical-record-templates`; `PUT /system/medical-record-templates/{id}`; `PATCH /system/medical-record-templates/{id}/default`, `/{id}/status` | `MEDICAL_RECORD_TEMPLATE_MANAGE` | NCL-13-CN-003: template belongs to one active specialty; normalized name is unique within specialty; sections use only canonical medical-record fields and unique display order; no hard delete; a specialty retains at least one active template; deactivating the default requires an active replacement in the same specialty. | Implemented — V29; seeded to `ADMIN` |
...
<<<<<<< HEAD
| `GET /cashier-shifts`, `/current-summary`, `/{id}`; `POST /cashier-shifts/close`; `POST /cashier-shifts/{id}/confirm` → `CashierShiftController` | `CASHIER_SHIFT_READ` / `CASHIER_SHIFT_CREATE` / `CASHIER_SHIFT_CONFIRM` | NCL-07-CN-009: Lễ tân chốt ca (`CASHIER_SHIFT_CREATE`), Quản lý phòng khám xác nhận (`CASHIER_SHIFT_CONFIRM`), xem tổng hợp ca/lịch sử (`CASHIER_SHIFT_READ`). Chặn hoàn tiền payment đã chốt (QTN-38, QTN-09) | V84; seeded to `RECEPTIONIST`, `MANAGER`, `ADMIN` |
=======
| `GET /cashier-shifts`, `/current-summary`, `/{id}`; `POST /cashier-shifts/close`; `POST /cashier-shifts/{id}/confirm` → `CashierShiftController` | `CASHIER_SHIFT_READ` / `CASHIER_SHIFT_CREATE` / `CASHIER_SHIFT_CONFIRM` | NCL-07-CN-009: Lễ tân chốt ca (`CASHIER_SHIFT_CREATE`), Quản lý phòng khám xác nhận (`CASHIER_SHIFT_CONFIRM`), xem tổng hợp ca/lịch sử (`CASHIER_SHIFT_READ`). Chặn hoàn tiền payment đã chốt (QTN-38, QTN-09) | V78; seeded to `RECEPTIONIST`, `MANAGER`, `ADMIN` |
