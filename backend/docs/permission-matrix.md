...
| `GET /cashier-shifts`, `/current-summary`, `/{id}`; `POST /cashier-shifts/close`; `POST /cashier-shifts/{id}/confirm` → `CashierShiftController` | `CASHIER_SHIFT_READ` / `CASHIER_SHIFT_CREATE` / `CASHIER_SHIFT_CONFIRM` | NCL-07-CN-009: Lễ tân chốt ca (`CASHIER_SHIFT_CREATE`), Quản lý phòng khám xác nhận (`CASHIER_SHIFT_CONFIRM`), xem tổng hợp ca/lịch sử (`CASHIER_SHIFT_READ`). Chặn hoàn tiền payment đã chốt (QTN-38, QTN-09) | V82; seeded to `RECEPTIONIST`, `MANAGER`, `ADMIN` |
