...
const PrivateRoute = ({ children, allowedRoles = [], allowedPermissions = [] }) => {
  ...
  return (
    <div style={{ padding: '60px 24px', textAlign: 'center', background: '#ffffff', minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Result
        status="403"
        title="403 - Giới hạn truy cập"
        subTitle="Tài khoản của bạn hiện tại chưa được cấp quyền để truy cập chức năng này."
        extra={
...
function AppRoutes() {
  return (
    <Routes>
      ...
      >
        <Route index element={<LazyPage><DashboardIndex /></LazyPage>} />
        ...
        <Route path="pharmacy" element={<PrivateRoute allowedPermissions={['PHARMACY_READ']} allowedRoles={['admin', 'pharmacist']}><LazyPage><PharmacyPage /></LazyPage></PrivateRoute>} />
        <Route path="pharmacy/receipts" element={<PrivateRoute allowedPermissions={['PHARMACY_CREATE']} allowedRoles={['admin', 'pharmacist']}><LazyPage><InventoryReceiptPage /></LazyPage></PrivateRoute>} />
        ...
        <Route path="system-management" element={<PrivateRoute allowedPermissions={['ROLE_READ', 'ROLE_UPDATE', 'CLINIC_CONFIGURATION_READ', 'USER_READ', 'AUDIT_READ', 'BACKUP_READ', 'SERVICE_CATALOG_READ']} allowedRoles={['admin', 'manager', 'clinic_manager']}><LazyPage><SystemManagementPage /></LazyPage></PrivateRoute>} />
