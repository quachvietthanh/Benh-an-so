...
export const getNavigationItems = (roles = [], permissions = []) => {
  ...
  const items = [
    ...
    { key: '/doctor-schedules', label: 'Lịch làm việc bác sĩ', icon: CalendarOutlined, check: () => !isReceptionist && !isPharmacist && (isAdmin || isManager || isDoctor || hasPerm('DOCTOR_SCHEDULE_UPDATE') || hasPerm('DOCTOR_TIMEOFF_CREATE')) },
